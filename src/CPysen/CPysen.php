<?php
/**
 * Main class for Pysen, holds everything.
 *
 * @package PysenCore
 */
class CPysen implements ISingleton {
        ...
        protected function __construct() {
                ...
                // include the site specific config.php and create a ref to $py to be used by config.php
                $py = &$this;
    require(PYSEN_SITE_PATH.'/config.php');
  ...
  /**
         ...
         * @return CPysen The instance of this class.
         */
        public static function Instance() {
                if(self::$instance == null) {
                        self::$instance = new CPysen();
                }
  ...
  public function ThemeEngineRender() {
    ...
    $themePath         = PYSEN_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = PYSEN_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl        = $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $py->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $py
    ...
    $py = &$this;
    // First the default Pysen themes/functions.php
    include(PYSEN_INSTALL_PATH . '/themes/functions.php');
    ...
    // Extract $py->data to own variables and handover to the template file
  ...
  /**
   * Draw HTML for a menu defined in $py->config['menus'].
   *
