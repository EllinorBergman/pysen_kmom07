 ...
 */
$py->config['debug']['pysen'] = false;
$py->config['debug']['session'] = false;
$py->config['debug']['timer'] = true;
$py->config['debug']['db-num-queries'] = true;
$py->config['debug']['db-queries'] = true;


/**
 * Set database(s).
 */
$py->config['database'][0]['dsn'] = 'sqlite:' . PYSEN_SITE_PATH . '/data/.ht.sqlite';
 ...
 */
$py->config['url_type'] = 1;


/**
 * Set a base_url to use another than the default calculated
 */
$py->config['base_url'] = null;


/**
 * How to hash password of new users, choose from: plain, md5salt, md5, sha1salt, sha1.
 */
$py->config['hashing_algorithm'] = 'sha1salt';


/**
 * Allow or disallow creation of new user accounts.
 */
$py->config['create_new_users'] = true;


/**
 * Define session name
 */
$py->config['session_name'] = preg_replace('/[:\.\/-_]/', '', __DIR__);
$py->config['session_key']  = 'pysen';


/**
 * Define default server timezone when displaying date and times to the user. All internals are still UTC.
 */
$py->config['timezone'] = 'Europe/Stockholm';


/**
 * Define internal character encoding
 */
$py->config['character_encoding'] = 'UTF-8';


/**
 * Define language
 */
$py->config['language'] = 'en';
 ...
 * Define menus.
 *
 * Create hardcoded menus and map them to a theme region through $py->config['theme'].
 */
$py->config['menus'] = array(
  'navbar' => array(
 ...
 * Template files can reside in the parent or current theme, the CPysen::ThemeEngineRender()
 ...
 *
 * path: Path to current theme, relativly PYSEN_INSTALL_PATH, for example themes/grid or site/themes/mytheme.
